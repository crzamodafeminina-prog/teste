...
export default function Admin({ dados, onSalvar, onVoltar }) {
  ...
  const restaurar = () => {
    if (
      ...
      localStorage.removeItem("crza:site-data:v1");
      ...
      <header className="sticky top-0 z-20 border-b border-[#071A33]/10 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
          ...
          <div className="flex gap-2">
            <Botao secondary onClick={onVoltar}>
              Ver loja
            </Botao>
          ...
          {aba === "inicio" && (
            <div className="space-y-6">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <Campo
                  label="Nome da marca"
                  value={local.config.marca}
                  onChange={(v) => config("marca", v)}
                />

                <Campo
                  label="Cidade"
                  value={local.config.cidade}
                  onChange={(v) => config("cidade", v)}
                />
              </div>

              <Campo
                label="Título principal da home"
                value={local.config.heroTitulo || ""}
                onChange={(v) => config("heroTitulo", v)}
              />

              <Area
                label="Texto principal da home"
                value={local.config.heroTexto || ""}
                onChange={(v) => config("heroTexto", v)}
                rows={4}
              />

              <Campo
                label="Título da seção institucional"
                value={local.config.identidadeTitulo || ""}
                onChange={(v) =>
                  config("identidadeTitulo", v)
                }
              />

              <Area
                label="Texto da seção institucional"
                value={local.config.identidadeTexto || ""}
                onChange={(v) =>
                  config("identidadeTexto", v)
                }
                rows={4}
              />
              ...
              <Campo
                ...
                value={local.config.colecaoTitulo || ""}
                onChange={(v) =>
                  config("colecaoTitulo", v)
                }
              />

              <Area
                label="Texto da coleção"
                value={local.config.colecaoTexto || ""}
                onChange={(v) =>
                  config("colecaoTexto", v)
                }
                rows={4}
              />
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <Campo
                  ...
                  value={local.config.whatsapp}
                  onChange={(v) => config("whatsapp", v)}
                />

                <Campo
                  label="Instagram (sem @)"
                  value={local.config.instagram}
                  onChange={(v) => config("instagram", v)}
                />

                <Campo
                  label="E-mail"
                  value={local.config.email}
                  onChange={(v) => config("email", v)}
                />
                ...
                <Campo
                  ...
                  type="number"
                  value={
                    Number(local.config.descontoPix || 0) * 100
                  }
                  onChange={(v) =>
                    config(
                      "descontoPix",
                ...
                <Campo
                  ...
                  type="number"
                  value={local.config.freteGratisAcima}
                  onChange={(v) =>
                    config(
                      "freteGratisAcima",
                      Number(v || 0)
              ...
              <Area
                ...
                value={
                  local.config.mensagemWhatsApp ||
                  "Oi! Vim pelo site da CRZA e tenho uma dúvida."
                }
                onChange={(v) =>
                  config("mensagemWhatsApp", v)
                }
                rows={3}
              />
            </div>
                ...
                {local.categorias.map((cat, i) => (
                  ...
                  >
                    <input
                      value={cat}
                      onChange={(e) => {
                        const a = [...local.categorias];
                        a[i] = e.target.value;
                        atualizar("categorias", a);
                      }}
            ...
            <div className="space-y-7">
              ...
              <div className="space-y-5">
                <Campo
                  ...
                  value={local.images.hero}
                  onChange={(v) => imagem("hero", v)}
                  placeholder="https://..."
                />

                <Campo
                  label="Imagem da seção Sobre"
                  value={local.images.sobre}
                  onChange={(v) => imagem("sobre", v)}
                  placeholder="https://..."
                />

                <Campo
                  label="Imagem da coleção"
                  value={local.images.colecao}
                  onChange={(v) => imagem("colecao", v)}
                  placeholder="https://..."
                />
              </div>
