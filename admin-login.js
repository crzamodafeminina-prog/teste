...
function cookieValue(cookieHeader) {
  const item = (cookieHeader || "").split(";").map(x => x.trim()).find(x => x.startsWith(`${COOKIE}=`));
...
export default function handler(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({ authenticated: validToken(cookieValue(req.headers.cookie)) });
  }
  ...
  if (!env("ADMIN_PASSWORD") || !env("ADMIN_SESSION_SECRET")) {
    return res.status(503).json({ error: "O acesso administrativo ainda não foi configurado na Vercel." });
  }
